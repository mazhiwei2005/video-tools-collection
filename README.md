...
| 工具 | 功能 | 路径 |
|------|------|------|
| **视频处理工具** | 套框/画中画/横转竖/流水线/合并/格式转换（GUI+Web双端） | `视频处理工具/` |
...
```
- 🖼️ 套框 — 为视频添加模板背景框，支持自定义ROI区域
- 📺 画中画 — 模板背景+多区域视频叠加，支持装饰视频
- 🔄 横转竖 — 横屏转竖屏，支持模糊/颜色/镜像背景
- 🎬 流水线 — 串联多步骤自动顺序处理
- 🔗 合并 — 多视频文件合并
- 🎞️ 格式转换 — 视频格式/编码转换
- ✨ 布局可视化预览
...
### v2.2.0 (2026-06-08)
- 🆕 画中画模式大幅升级：视频模板、PR风格效果控件、区域/装饰复选框、片头片尾独立模式
- 🆕 分批次输出：按集数分割文件夹（26集按11集分割→3个文件）
- 🔧 NVENC时长修复（预裁剪模板方案）、临时文件自动清理、UI优化
- 🐛 修复图片模板时长异常、stream_loop时长bug、CTkLabel崩溃

### v2.1.0 (2026-06-07)
- 🆕 流水线模式、合并视频、布局可视化预览
- 🔧 CTk全面UI重构、窗口比例优化、视频列表固定
- 🐛 修复CTkButton/config兼容性、中文路径编码、Checkbox不可见
